...
DATASET_PATH = "endb_human_clean.csv"


def pipeline_endb_rag_qasar():
    qa_plugin = QualityQASAR(api_key=QASAR_API_KEY)
    qa_config = QASARConfig(
        persistent=False,
        ontology_name="ENdb Human Ontology - RAG + QASAR"
    )
    ...
    complete_diagram = engine.merge_puml_diagram(diagrams) if len(diagrams) > 1 else diagrams[0]

    with open("endb_rag_qasar.puml", "w", encoding="utf-8") as file:
    ...
    with open("endb_rag_qasar_mappings.json", "w", encoding="utf-8") as file:
    ...
    with open("endb_rag_qasar.ttl", "w", encoding="utf-8") as file:
    ...
    with open("endb_rag_qasar_report.json", "w", encoding="utf-8") as file:
