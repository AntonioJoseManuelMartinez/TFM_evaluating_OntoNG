...
def pipeline_endb_no_rag_qasar():
    qa_plugin = QualityQASAR(api_key=QASAR_API_KEY)
    qa_config = QASARConfig(
        persistent=False,
        ontology_name="ENdb Human Ontology - no RAG + QASAR"
    )
    ...
    with open("endb_no_rag_qasar.puml", "w", encoding="utf-8") as file:
    ...
    with open("endb_no_rag_qasar.ttl", "w", encoding="utf-8") as file:
        file.write(ontology)
    ...
    with open("endb_no_rag_qasar_report.json", "w", encoding="utf-8") as file:
