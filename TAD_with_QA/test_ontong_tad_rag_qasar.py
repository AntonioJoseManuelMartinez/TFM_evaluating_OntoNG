import os
import json
from OntoNG import OntoNG, LLMConfig, RAGTecnomod, EnginePlugins, QASARConfig, QualityQASAR
...
DATASET_PATH = "tad_3files.csv"   # cambia la ruta si tu CSV está en otro sitio


def pipeline_tad_rag_qasar():
    qa_plugin = QualityQASAR(api_key=QASAR_API_KEY)
    qa_config = QASARConfig(
        persistent=False,
        ontology_name="TAD Ontology - RAG + QASAR"
    )

    rag_plugin = RAGTecnomod()
    ...
    # Mantenemos el patrón del ejemplo oficial, pero si no hay partición
    # partition_data devolverá el dataset original.
    ...
    with open("tad_rag_qasar.puml", "w", encoding="utf-8") as file:
        file.write(complete_diagram)

    print("=== DIAGRAM GENERATED ===")
    print(complete_diagram)

    # Para este dataset TAD el diagrama es plano; evitamos buscar relaciones
    # porque el endpoint de similar-relations está devolviendo 400.
    try:
        mappings = engine.query_rag(
            text=complete_diagram,
            blacklist=[],
            enable_terms_lookup=True,
            enable_properties_lookup=False,
            enable_relations_lookup=False,
            top_k=10
        )
    except Exception as e:
        print("=== RAG QUERY ERROR ===")
        print(str(e))
        mappings = {
            "mapped_terms": {},
            "mapped_relations": {},
    ...
    with open("tad_rag_qasar_mappings.json", "w", encoding="utf-8") as file:
    ...
    with open("tad_rag_qasar.ttl", "w", encoding="utf-8") as file:
        file.write(rag_ontology)

    print("=== ONTOLOGY GENERATED ===")
    print(rag_ontology[:3000])

    report = qa_plugin.analyze(ontology=rag_ontology, config=qa_config)

    with open("tad_rag_qasar_report.json", "w", encoding="utf-8") as file:
