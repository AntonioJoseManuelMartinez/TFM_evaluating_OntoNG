...
DATASET_PATH = "tad_all_files.csv"


def pipeline_tad_all_rag_qasar():
    qa_plugin = QualityQASAR(api_key=QASAR_API_KEY)
    qa_config = QASARConfig(
        persistent=False,
        ontology_name="TAD All Files Ontology - RAG + QASAR"
    )
    ...
    with open("tad_all_rag_qasar.puml", "w", encoding="utf-8") as file:
    ...
    with open("tad_all_rag_qasar_mappings.json", "w", encoding="utf-8") as file:
    ...
    with open("tad_all_rag_qasar.ttl", "w", encoding="utf-8") as file:
    ...
    with open("tad_all_rag_qasar_report.json", "w", encoding="utf-8") as file:
